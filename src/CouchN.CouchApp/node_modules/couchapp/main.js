...
function createApp(doc, url, cb) {
    ...
    var push = function (callback) {
        ...
        var filename = doc.exportFilename;
        
        if (!filename)
            throw { name: "Error", message: "the design document must have an 'exportFilename' property set." };
        ...
        fs.writeFile(filename, body, function (err) {
            if (err) {
                ...
                console.log(doc._id + " saved to: " + filename);
            }
        });
    };

    app.push = function (callback) {
        var revpos, pending_dirs = 0;
                ...
                for (i in files) {
                    (function (f) {
                        fs.readFile(f, function (err, data) {
                            ...
                            if (f[0] == '/') f = f.slice(1);
                            if (!err) {
                                var d = data.toString('base64'), md5 = crypto.createHash('md5'), mime = mimetypes.lookup(path.extname(f).slice(1));
                                md5.update(d);
                                md5 = md5.digest('hex');
